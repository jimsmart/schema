...
var _ = Describe("Using driver github.com/jbarham/gopgsqldriver (postgres)", func() {
	...
	// TODO(js) This driver does not currently pass tests (github.com/jbarham/gopgsqldriver)

	var params = drivers.PostgresDialect
	params.DriverName = "postgres"
	params.ConnStr = fmt.Sprintf("user=%s host=%s port=%s sslmode=disable dbname=postgres", user, host, port)
